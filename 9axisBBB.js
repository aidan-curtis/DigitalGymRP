...
const machineID = 117;						// machine ID shoulb be unique for each sensor tag
...
SensorTag.discover(function(tag) {			//on discovery the function defined inside the parentheses (a callback) will be run
	...
	function notifyMe() {
   	...
   	function listenFor9Change(){
   		tag.on('9axisChange', function(x, y, z, xG, yG, zG, xM, yM, zM){
			...
			buffer[String(time)] = [x, y, z, xG, yG, zG, xM, yM, zM, machineID];
	...
	function sendData(buff){  //post request
		...
		request.post({url : 'http://35.162.184.130:8000/bbb/bike', json: buff}, function optionalCallback(err, httpResponse, body){
			if (err) {
